...
class Camera(Sensor):
    def __init__(self, vehicle, world, actor_list, folder_output, transform, config):
        ...
        os.makedirs(self.frame_output) if not os.path.exists(self.frame_output) else [os.remove(f) for f in glob.glob(
            self.frame_output + "/*") if os.path.isfile(f)]
    ...
    def save(self, color_converter=carla.ColorConverter.Raw, no_id=False):
        while not self.queue.empty():
            ...
            if ts - self.ts_tmp > self.capture_period * 1.5 or (
                    ts - self.ts_tmp) < 0:  # check for 10Hz camera acquisition
...
class SS(Camera):
    ...
    def set_attributes(self, blueprint_library):
        ...
        camera_ss_bp.set_attribute('fov',
                                   '72')  # 72 degrees # Always fov on width even if width is different than height
    ...
    def save(self):
        while not self.queue.empty():
            ...
            if self.i_packet % self.packet_per_frame == 0:
                ...
                if ply.write_ply(ply_file_path, [np.float32(pts_all), np.float32(ts_all), np.uint32(semantic_all)],
                                 ['x', 'y', 'z', 'cos_angle_lidar_surface', 'timestamp', 'instance', 'semantic']):
                    if _DEBUG:
...
class SemanticLidar(Sensor):
    ...
    def __init__(self, vehicle, world, transform, config):
        self.rotation_lidar = rotation_carla(transform.rotation)
        self.rotation_lidar_transpose = self.rotation_lidar.T
        self.queue = queue.PriorityQueue()
        self.bp = self.set_attributes(world.get_blueprint_library())
    ...
    def save(self, tick):
        ...
        for k in keys:
            if tick - self.pedestrians[k] > 2:
                del self.pedestrians[k]
        keys = list(self.vehicles.keys())
        for k in keys:
            if tick - self.vehicles[k] > 2:
                del self.vehicles[k]
...
def transform_lidar_to_camera(lidar_tranform, camera_transform):
    ...
    R_lidar_vehicle = np.identity(
        3)  # rotation_carla(lidar_tranform.rotation) #we want the lidar frame to have x forward
    R_lidar_camera = R_camera_vehicle.T.dot(R_lidar_vehicle)
    T_lidar_camera = R_camera_vehicle.T.dot(translation_carla(
        np.array([[lidar_tranform.location.x], [lidar_tranform.location.y], [lidar_tranform.location.z]]) - np.array(
