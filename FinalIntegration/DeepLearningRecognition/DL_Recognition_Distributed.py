...
class RecognitionWorker(multiprocessing.Process):
    def __init__(self, id, sensor, parent: DistributedRecognition):
        self._logger = logging.getLogger()
        self._id = id
        self._logger.info(f"Loading Recognition worker - {id}")
        ...
        self._model = self._load_model(parent.config.get("model_path",""), parent.config.get("hubconf_path",""))
        self._model.eval()
